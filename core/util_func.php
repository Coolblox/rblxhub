...
function StoreCachedAvatar($avatar) {
  ...
  if (mysqli_num_rows(mysqli_query($connect, "SELECT * FROM `avatar_cache` WHERE `hash`='$hash'")) < 1) {
    mysqli_query($connect, $stmt) or die("ERR:".mysqli_error($connect));
...
function GenerateAvatarThumbnail($hash) {
  ...
  $chck = mysqli_query($connect, "SELECT * FROM `avatar_cache` WHERE `hash`='$hash'") or die("ERR:".mysqli_error($connect));
...
function LogUserAction($_USER, $type, $info) {
  ...
  if (!in_array($type, $valid_actions)) {
    ...
    $info = $connect->real_escape_string(htmlspecialchars($info));
    $time = time();
    $ip = md5($_SERVER['REMOTE_ADDR']);
    $connect->query("INSERT INTO `user_action_log`(`user_id`, `action_type`, `info`, `ip_address`, `time`) VALUES ('{$_USER['id']}', '$type', '$info', '$ip', '$time')") or die($connect->error);
  }
...
function SendAutomatedMessageToId($subject, $content, $uid) {
  ...
  (`id`, `user_from`, `user_to`, `subject`, `content`, `datesent`) VALUES (
    ...
    '$currenttimelol')";
                  //echo ($stmt);
                  $q = mysqli_query($connect, $stmt) or die(mysqli_error($connect));
}
