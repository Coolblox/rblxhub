...
<div id="body">
    <table width="100%" cellspacing="0" cellpadding="0" border="0">
        ...
        if (isset($_POST['title'])) {
          $title = htmlspecialchars(mysqli_real_escape_string($connect, $_POST['title']));
          $content = htmlspecialchars(mysqli_real_escape_string($connect, $_POST['content']));
          $captcha_completed=true;
          if (!$captcha_completed) {
            echo "reCAPTCHA failed, please try again.";
          } else {
            if (FilterString($title) != "OK") {
              $p = FilterString($title);
              echo "Your title contains profanity, please remove \"$p\" and try again!";
            } else {
              if (FilterString($content) != "OK") {
                $p = FilterString($content);
                echo "Your forum post contains profanity, please remove \"$p\" and try again!";
              } else {
                //Post it
                ...
                mysqli_query($connect, $stmt) or die(mysqli_error($connect));
                $insid = mysqli_insert_id($connect);
