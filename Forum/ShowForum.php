...
$threadsperpage = 20;
$total = mysqli_num_rows(mysqli_query($connect, "SELECT id FROM forum WHERE category='$topic' AND reply_to='0' ORDER BY is_pinned DESC, time_posted DESC"));
...
$fq = mysqli_query($connect, "SELECT * FROM forum WHERE category='$topic' AND reply_to='0' ORDER BY is_pinned DESC, time_posted DESC") or die(mysqli_error($connect));
?>
 <div id="Body">
     <style>
/*****************************************************
    ...
    background-color: #DDEEFF;
	    ...
	    <?php
                while ($post = mysqli_fetch_assoc($fq)) {
                  ...
                  $replies = mysqli_num_rows(mysqli_query($connect, "SELECT id FROM forum WHERE reply_to='{$post['id']}'"));
                  if ($replies > 0) {
                    $lastreply = mysqli_fetch_assoc(mysqli_query($connect, "SELECT * FROM forum WHERE reply_to='{$post['id']}' ORDER BY id DESC LIMIT 1"));
                    $lrtimeago = time_elapsed_string("@{$lastreply['time_posted']}");
                    $lrauthor = mysqli_fetch_assoc(mysqli_query($connect, "SELECT * FROM users WHERE id='{$lastreply['author']}'"));
                    $lrstring = "$lrtimeago <br> by <a href=\"/User.php?id={$lrauthor['id']}\">{$lrauthor['username']}</a>";
                  } else {
                    $lrstring = "";
                  }
                  echo "<tr>
                    <td style=\"max-width: 60px;\"><img src=\"/Forum/api/skins/default/images/topic_notread.gif\" ></td>
                    <td class=\"forumRow\" height=\"25\"><a class=\"linkSmallBold\" href=\"/Forum/ShowPost.php?id={$post['id']}\">{$post['title']}</a></td>
